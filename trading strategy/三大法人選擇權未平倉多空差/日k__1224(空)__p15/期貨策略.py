...
sql = "SELECT table_a.date,\
        SUM(table_a.long_oi_volume + table_b.short_oi_volume) AS long,\
        SUM(table_b.long_oi_volume + table_a.short_oi_volume) AS short\
        FROM api_taifex_big3_options as table_a\
        JOIN api_taifex_big3_options as table_b\
        ON table_a.date=table_b.date AND table_a.type=table_b.type\
        WHERE table_b.long_oi_volume+table_a.short_oi_volume!=0 AND table_a.cp='c' AND table_a.cp!=table_b.cp\
...
threebig_mc_diff.to_csv('big3_diff_%d.csv' %days, index=False)
